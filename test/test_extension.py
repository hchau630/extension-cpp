...
class TestMyExp(TestCase):
    def sample_inputs(self, device, *, requires_grad=False):
        ...
        return [
            ...
            [make_nondiff_tensor(2, 3) + 1j],
        ]

    def _test_correctness(self, device):
        samples = self.sample_inputs(device)
        for args in samples:
            result = extension_cpp.ops.myexp(*args)
            expected = torch.exp(*args)
    ...
    def _opcheck(self, device):
        ...
        for args in samples:
            opcheck(torch.ops.extension_cpp.myexp.default, args)
...
class TestMyMulAdd(TestCase):
    def sample_inputs(self, device, *, requires_grad=False):
        ...
        return [
            ...
            [make_tensor(20), make_nondiff_tensor(20), make_nondiff_tensor(1).half()],
            [make_nondiff_tensor(2, 1), make_tensor(2, 3), make_tensor(2, 3).double()],
        ]
